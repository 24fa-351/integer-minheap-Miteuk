...
heap_t *heap_create(int capacity) {
    ...
    if (!heap->data) {
        free(heap);
        printf("Memory allocation for heap nodes failed\n");
        return NULL;
    }
    heap->size = 0;
...
void heap_bubble_down(heap_t *heap, int index) {
     while (1) {
        ...
        if (left_child < heap->size && heap->data[left_child].key < heap->data[smallest].key) {
            smallest = left_child;
        }
        if (right_child < heap->size && heap->data[right_child].key < heap->data[smallest].key) {
